...
class Rifle(Weapon):
    ...
    def __init__(self):
        ...
        self.gun.muzzle_flash = Entity(parent=camera, position=(0.95,-.4,4.5), scale=0.3, model='icosphere', color=color.yellow, enabled=False)
...
class Handgun(Weapon):
    ...
    def __init__(self):
        super().__init__()
        self.gun = Entity(model='guns/handgun.obj', parent=camera, position=(.8,-.3,.5), scale=1, rotation_y=-90, origin_z=-.5, texture='guns/handgun', on_cooldown=False)
        self.gun.muzzle_flash = Entity(parent=camera, position=(1.4,-.9,4.5), scale=0.5, model='icosphere', color=color.yellow, enabled=False)
    ...
    def _single_action(self):
        ...
        if not gun.on_cooldown:
            ...
            gun.muzzle_flash.enabled = True
            from ursina.prefabs.ursfx import ursfx
            ursfx([(0.0, 0.0), (0.1, 0.9), (0.15, 0.75), (0.3, 0.14), (0.6, 0.0)], volume=1, wave='noise', pitch=random.uniform(-58,-57), pitch_change=-12, speed=3.0)
            invoke(gun.muzzle_flash.disable, delay=.05)
            invoke(setattr, gun, 'on_cooldown', False, delay=.15)
            if mouse.hovered_entity and hasattr(mouse.hovered_entity, 'hp'):
...
class Railgun(Weapon):
    ...
    def __init__(self):
        super().__init__()
        self.gun = Entity(model='guns/railgun.obj', parent=camera, position=(6,-2,-6), scale=10, rotation_y=-90, origin_z=-.5, texture='guns/railgun.jpg', on_cooldown=False, double_sided=True)
        self.gun.muzzle_flash = Entity(parent=camera, position=(1.4,-1.2,4.5), scale=1.5, model='icosphere', color=color.blue, enabled=False)
    ...
    def _single_action(self):
        ...
        if not gun.on_cooldown:
            ...
            gun.muzzle_flash.enabled = True
            from ursina.prefabs.ursfx import ursfx
            ursfx([(0.0, 0.0), (0.1, 0.9), (0.15, 0.75), (0.3, 0.14), (0.6, 0.0)], volume=1.5, wave='triangle', pitch=random.uniform(-3,-1), pitch_change=-12, speed=1.0)
            invoke(gun.muzzle_flash.disable, delay=.05)
            invoke(setattr, gun, 'on_cooldown', False, delay=.6)
            if mouse.hovered_entity and hasattr(mouse.hovered_entity, 'hp'):
                mouse.hovered_entity.hp -= 1000
                mouse.hovered_entity.blink(color.blue)
